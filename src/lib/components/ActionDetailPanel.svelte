...
{#if mobileState.isMobile}
	...
	<div
		class="fixed inset-0 z-40 bg-white dark:bg-gray-950 overflow-y-auto"
		transition:slide={{ duration: 200 }}
	>
		<div class="p-4">
			<!-- Header with close button -->
			<div class="flex items-center justify-between mb-4 sticky top-0 bg-white dark:bg-gray-950 pb-2 border-b border-gray-200 dark:border-gray-800">
				<h4 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
					Action Details
				</h4>
				<button
					onclick={onClose}
					class="min-w-11 min-h-11 flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
					title="Close"
				>
			...
			<div class="space-y-4">
				...
				<div>
					...
					<input
						type="text"
						bind:value={title}
						class="w-full px-3 py-2 text-base border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-transparent"
					/>
				</div>
				...
				<div>
					...
					<textarea
						bind:value={notes}
						rows="5"
						class="w-full px-3 py-2 text-base border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-transparent resize-y"
				...
				<div>
					...
					<select
						...
						class="w-full px-3 py-2 text-base border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-transparent"
					>
				...
				<!-- Metadata (read-only) -->
				<div class="flex flex-col gap-2 text-sm text-gray-500 dark:text-gray-400 pt-2 border-t border-gray-200 dark:border-gray-700">
					<div>
				...
				<!-- Action buttons -->
				<div class="flex flex-col gap-2 pt-2 sticky bottom-0 bg-white dark:bg-gray-950 pb-4">
					<button
						onclick={handleSave}
						class="w-full min-h-11 px-4 py-2.5 text-base font-medium text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:ring-offset-2"
					>
						Save
					</button>
					<button
						onclick={handleDelete}
						class="w-full min-h-11 px-4 py-2.5 text-base font-medium text-white bg-red-600 hover:bg-red-700 dark:bg-red-500 dark:hover:bg-red-600 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-red-500/40 focus:ring-offset-2"
					>
						Delete
					</button>
				</div>
	...
	<div class="space-y-4">
		...
		<div>
			...
			<select
				bind:value={context}
				onchange={handleContextChange}
				class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-transparent"
			>
				<option value="">No context</option>
				{#each actionState.contexts as ctx (ctx.id)}
					<option value={ctx.name}>{ctx.name}</option>
